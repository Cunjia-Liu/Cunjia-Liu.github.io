...
for row, item in projects.iterrows():
    ...
    if len(str(item.funder)) > 3:
        md += 'funder: "' + item.funder + '"\n'
        
    if len(str(item.startdate)) > 3:
        md += "startdate: " + str(item.startdate) + "\n"
    if len(str(item.enddate)) > 3:
        md += "enddate: " + str(item.enddate) + "\n"
        
    if len(str(item.value)) > 1:
        md += 'value: "' + '£' + str(item.value) + 'k' + '"\n'
    if len(str(item.ID)) > 3:
        md += 'ID: "' + str(item.ID) + '"\n'  
    md += "---\n"
